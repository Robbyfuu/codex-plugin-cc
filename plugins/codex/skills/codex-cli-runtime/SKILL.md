...
Use this skill only inside the `codex-plus:codex-rescue` subagent.
...
- Do not call `setup`, `review`, `adversarial-review`, `status`, `result`, or `cancel` from `codex-plus:codex-rescue`.
...
Safety rules:
- Default to write-capable Codex work in `codex-plus:codex-rescue` unless the user explicitly asks for read-only behavior.
