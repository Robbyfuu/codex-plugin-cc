...
Use this skill only inside the `peer:codex-rescue` subagent.
...
- Do not call `setup`, `review`, `adversarial-review`, `status`, `result`, or `cancel` from `peer:codex-rescue`.
...
Safety rules:
- Default to write-capable Codex work in `peer:codex-rescue` unless the user explicitly asks for read-only behavior.
