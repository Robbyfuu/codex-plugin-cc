...
function buildCodexProcessOptions(env = process.env) {
  const sanitizedEnv = sanitizeCodexSpawnEnv(env);
  return { cwd: ensureAppServerRuntimeDir(sanitizedEnv), env: sanitizedEnv };
}
...
async function withDirectAppServer(cwd, fn, options = {}) {
  const env = options.env;
  const connectImpl = options.connectImpl ?? CodexAppServerClient.connect.bind(CodexAppServerClient);
  const client = await connectImpl(cwd, { env, disableBroker: true });
  try {
...
function importedThreadIdForSource(sourcePath, env = process.env) {
  const ledgerPath = path.join(resolveCodexHome(env), "external_agent_session_imports.json");
...
export function getCodexAvailability(_cwd) {
  const processOptions = buildCodexProcessOptions();
  const versionStatus = binaryAvailable("codex", ["--version"], processOptions);
  if (!versionStatus.available) {
    return versionStatus;
  }

  const appServerStatus = binaryAvailable("codex", ["app-server", "--help"], processOptions);
...
export async function importExternalAgentSession(cwd, options = {}) {
  ...
  return withDirectAppServer(cwd, async (client) => {
    ...
    const threadId = importedThreadIdForSource(options.sourcePath, options.env ?? process.env);
    if (!threadId) {
