...
- `CODEX_COMPANION_WATCH_PANE` (default: enabled when `TMUX` is set): when running a job inside tmux, automatically open a split pane that tails the job's live log. Set to `0`/`false`/`off`/`no` to disable, or `1`/`true`/`on`/`yes` to force-enable. Use `/codex:watch [job-id]` to open the pane manually.
