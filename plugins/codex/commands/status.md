...
For active (queued or running) jobs, the output includes a `Live log:` line with the absolute path to the per-job event log, plus a `Tail:` hint (`tail -F <path>`) and a `Watch pane:` hint (`/codex-plus:watch <job-id>`). Preserve these so the user can follow the turn live as it happens instead of waiting for it to finish.
