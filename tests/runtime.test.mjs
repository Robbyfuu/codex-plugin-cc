...
import { cleanupTrackedCodexProcesses, initGitRepo, makeTempDir, run } from "./helpers.mjs";
...
test("transfer delegates the current Claude session directly to native import", () => {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      { type: "user", cwd: repo, message: { role: "user", content: "/peer:transfer" } }
  ...
  assert.deepEqual(
    ...
    ["Initial request", "Initial answer", "/peer:transfer"]
  );
});
...
test("review rejects focus text because it is native-review only", () => {
  ...
  assert.match(result.stderr, /\/peer:adversarial-review focus on auth/i);
});
...
test("status shows phases, hints, and the latest finished job", () => {
  ...
  const result = run("node", [SCRIPT, "status"], {
    cwd: workspace,
    env: { ...process.env, CODEX_COMPANION_SESSION_ID: "" }
  });
  ...
  assert.match(result.stdout, /`\/peer:status review-live`<br>`\/peer:cancel review-live`/);
...
test("stop hook logs running tasks to stderr without blocking when the review gate is disabled", () => {
  ...
  assert.match(blocked.stderr, /\/peer:status/i);
  assert.match(blocked.stderr, /\/peer:cancel task-live/i);
});
...
test("stop hook does not block when Codex is unavailable even if the review gate is enabled", () => {
  ...
  assert.match(allowed.stderr, /Run \/peer:setup/i);
});
